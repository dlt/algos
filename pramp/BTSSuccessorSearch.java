...
class Solution {
    ...
    static class BinarySearchTree {
        ...
        Node findInOrderSuccessor(Node inputNode) {
            ...
            convertToList(this.root, nodelist);


            int index = nodelist.indexOf(inputNode);

            if (index == nodelist.size() - 1) return null;

            return nodelist.get(index + 1);

        }

        void convertToList(Node node, ArrayList<Node> nodes) {
            if (node == null) return;

            convertToList(node.left, nodes);
            nodes.add(node);
            convertToList(node.right, nodes);
        }
